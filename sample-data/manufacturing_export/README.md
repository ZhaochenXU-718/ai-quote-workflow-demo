...
scripts/
  generate_manufacturing_export_data.mjs
```
...
| `../../scripts/generate_manufacturing_export_data.mjs` | Reproducible data generator | Regenerating inquiries and gold labels |
...
```text
scripts/generate_manufacturing_export_data.mjs
```

Run from repository root:

```bash
node scripts/generate_manufacturing_export_data.mjs
```
...
- Only English inquiries are included.
- Product data covers 12 SKUs, not the planned 30 SKUs.
- Historical quotation data has not been generated yet.
- There is no train/dev/eval split yet.
- Gold labels are generated by rule-based script and should be manually reviewed before serious evaluation.
...
4. Add `dev` and `eval` splits.
5. Add multilingual inquiries.
6. Add attachment-style inquiries that reference a product table or spec sheet.
7. Build the first parser/retrieval/risk-check pipeline against this dataset.
